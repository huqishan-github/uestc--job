...
- `reports/YYYY-MM-DD.json`：每次程序成功运行后生成的结构化当日变更数据，便于后续程序、自动筛选和岗位匹配读取。

JSON 日报直接从 `data/jobs.csv` 生成，不依赖对 Markdown 的二次解析。顶层包含 `schema_version`、`date`、`generated_at`、`source`、`summary` 和 `records`。其中 `records` 仅保留目标日期首次获取或发生更新的记录，并增加 `变更类型` 字段（`新增` / `更新`）；每条记录同时保留 CSV 中的唯一 ID、公司、岗位、地点、学历、专业、投递链接、详情链接等原始结构化字段。
...
CSV 和 Excel 都先写入同目录临时文件，校验记录数和唯一 ID 后再原子替换。网站整体不可访问时，程序输出“本次抓取失败，历史数据未修改”。JSON 日报同样采用临时文件校验后原子替换，避免生成半截文件。
...
- JSON 日报生成失败：程序返回失败状态，避免自动任务把不完整的日报当作正常结果提交。
